...
void PMDWIN::ch3mode_set(QQ *qq)
{
	...
	//	効果音モードに移った??合はそ??以前のFM3パートで音程書き換え
...
uchar PMDWIN::calc_panout(QQ *qq)
{
	...
	dl = qq->fmpan;
	if(qq->hldelay_c) dl &= 0xc0;	// HLFO Delayが残って????合はパンのみ設??
	return dl;
}
...
void PMDWIN::lfoinit(QQ *qq, int al)
{
	...
	if(ah == 0x0f) {				// キューフ ノ トキ ハ INIT シナイ??
		lfo_exit(qq);
		return;
	}
	
	qq->porta_num = 0;				// ポ??タメントは初??化
...
void PMDWIN::volset(QQ *qq)
{
	...
	if(qq != &EffPart) {	// 効果音の??合はvoldown/fadeout影響無し
		//--------------------------------------------------------------------
		//	音量down計算
		//--------------------------------------------------------------------
	...
	cl = 255-cl;			// cl=carrierに設定す??音量+80H(add)
	bl &= qq->carrier;		// bl=音量を設定す??SLOT xxxx0000b
	bh |= bl;
	...
	if(cl != 255) {
		if(qq->lfoswi & 2) {
			bl = qq->volmask;
			bl &= qq->slotmask;		// bl=音量LFOを設定す??SLOT xxxx0000b
			bh |= bl;
...
void PMDWIN::psgmain(QQ *qq)
{
	...
	if(qq == &SSGPart[2] && pmdwork.ppsdrv_flag &&
					...
					// PPS 使用?? & SSG 3ch & SSG 効果音鳴らしてい??????
		keyoffp(qq);
		opna.SetReg(pmdwork.partb+8-1, 0);		// 強制的に音を止め??
		qq->keyoff_flag = -1;			
	}
...
uchar * PMDWIN::rhythmon(QQ *qq, uchar *bx, int al, int *result)
{
	...
	*result = 1;
	...
	if(open_work.fadeout_volume) {
		...
		if(pmdwork.ppsdrv_flag == false) {	// fadeout??ppsdrvでなら発音しない
			bx = open_work.rhyadr;
			return bx;
		}
	}
	...
	do {
		...
		effgo(qq, al);
		bx_ >>= 1;
	}while(pmdwork.ppsdrv_flag && bx_);	// PPSDRVなら２音目以上も鳴らしてみ??
...
void PMDWIN::effgo(QQ *qq, int al)
{
	...
	effwork.hosei_flag = 3;				//	音??/音量補正あ?? (K part) 
	eff_main(qq, al);
}
...
{
	effwork.hosei_flag = 1;				//	音程のみ補正あ?? (n command)
	eff_main(qq, al);
}
...
void PMDWIN::eff_main(QQ *qq, int al)
{
	...
	if(pmdwork.ppsdrv_flag && (al & 0x80)) {	// PPS を鳴らす
		if(effwork.effon >= 2) return;	// 通??効果音発音??は発声させない
		
		SSGPart[2].partmask |= 2;		// Part Mask
		effwork.effon = 1;				// 優先度１(ppsdrv)
		effwork.psgefcnum = al;			// 音色番号設?? (80H~)
...
void PMDWIN::efffor(const int *si)
{
	...
	if(al == -1) {
		...
		effwork.effcnt = al;		// カウント??
		cl = *si;
		opna.SetReg(4, *si++);		// ??波数セット
		ch = *si;
		opna.SetReg(5, *si++);		// ??波数セット
		...
		opna.SetReg(10, *si++);		// ボ??ュー??
		opna.SetReg(11, *si++);		// エンベ?晙璽彈?波??
		opna.SetReg(12, *si++);		// 
		opna.SetReg(13, *si++);		// エンベ?晙璽?PATTERN
		
		effwork.eswtst = *si++;		// スイープ??分 (TONE)
		effwork.eswnst = *si++;		// スイープ??分 (NOISE)
...
void PMDWIN::otodasim(QQ *qq)
{
	...
	dx *= 4;	// PCM ハ LFO ガ カカ??ニクイ ノデ depth ?? 4バイ ス??
...
uchar * PMDWIN::portam(QQ *qq, uchar *si)
{
	...
	al_ = qq->onkai;
	fnumsetm(qq, oshift(qq, *si++));
	ax = qq->fnum; 			// ax = ポ??タメント先のdelta_n値
	
	qq->onkai = al_;
	qq->fnum = bx_;			// bx = ポ??タメント元のdekta_n値
	ax -= bx_;				// ax = delta_n差
...
uchar * PMDWIN::comat(QQ *qq, uchar *si)
{
	...
	if(qq->partmask == 0) {	// パートマスクさ??てい??か？
		...
		//	FM3chで、マスクさ??ていた??合、fm3_alg_fbを設??
...
void PMDWIN::neiroset(QQ *qq, int dl)
{
	...
	if(pmdwork.partb == 3 && pmdwork.fmsel == 0) {
		if(pmdwork.af_check != 0) {	// ALG/FBは設定しないmodeか？
			...
			if((qq->slotmask & 0x10) == 0) {	// slot1を使用してい??か？
...
uchar * PMDWIN::porta(QQ *qq, uchar *si)
{
	...
	cl = qq->onkai;
	fnumset(qq, oshift(qq, *si++));
	bx = qq->fnum;			// bx=ポ??タメント先のfnum値
	qq->onkai = cl;
	qq->fnum = cx;			// cx=ポ??タメント元のfnum値
...
uchar * PMDWIN::slotmask_set(QQ *qq, uchar *si)
{
	...
	if(qq->slotmask != ah) {
		qq->slotmask = ah;
		if((ah & 0xf0) == 0) {
			qq->partmask |= 0x20;	// s0の??パートマスク
		} else {
			qq->partmask &= 0xdf;	// s0以外の??パートマスク解??
		}
		
		if(ch3_setting(qq)) {		// FM3chの??合のみ ch3modeの変更処??
			// ch3なら、そ??以前のFM3パートでkeyon処??
			if(qq != &FMPart[2]) {
...
uchar * PMDWIN::ppz_extpartset(QQ *qq, uchar *si)
{
	...
	for(i = 0; i < 8; i++) {
		...
		if(ax) {
			...
			PPZ8Part[i].keyoff_flag = -1;			// 現在keyoff??
...
uchar * PMDWIN::fb_set(QQ *qq, uchar *si)
{
	...
	al = read_char(si++);
	if(al >= 0) {
		// in	al 00000xxx 設定す??FB
		al = ((al << 3) & 0xff) | (al >> 5);
		
		// in	al 00xxx000 設定す??FB
		if(pmdwork.partb == 3 && pmdwork.fmsel == 0) {
...
//	COMMAND 't±' [TEMPO CHANGE ??対1]
//	COMMAND 'T±' [TEMPO CHANGE ??対2]
//=============================================================================
...
uchar * PMDWIN::portap(QQ *qq, uchar *si)
{
	...
	al_ = qq->onkai;
	fnumsetp(qq, oshiftp(qq, *si++));
	ax = qq->fnum; 			// ax = ポ??タメント先のpsg_tune値
	
	qq->onkai = al_;
	qq->fnum = bx_;			// bx = ポ??タメント元のpsg_tune値
	ax -= bx_;
...
//=============================================================================
//	'w' COMMAND [PSG NOISE ヘイキ?? シュウハスウ]
//=============================================================================
...
{
	if((qq->lfoswi & 3) != 0) {		// 前が & の???? -> 1?? LFO処??
		lfo(qq);
	}
	
	if((qq->lfoswi & 0x30) != 0) {	// 前が & の???? -> 1?? LFO処??
		lfo_change(qq);
		lfo(qq);
		lfo_change(qq);
	}
...
void PMDWIN::lfoinit_main(QQ *qq)
{
	...
	if(qq->lfo_wave == 2 || qq->lfo_wave == 3) {	// ??形波 or ランダム波？
		qq->speed = 1;	// delay直後にLFOが掛か??ようにす??
	} else {
		qq->speed++;	// そ??以外の??合はdelay直後のspeed値?? +1
	}
}
...
void PMDWIN::fnumsetp(QQ *qq, int al)
{
	...
	if((al & 0x0f) == 0x0f) {		// キュウフ ナ?? FNUM ニ 0 ?? セット
		qq->onkai = 255;
...
int PMDWIN::lfop(QQ *qq)
{
	...
	if(qq->extendmode & 2) {	// TimerAと合??せ??か？
								// そうじゃないなら無条??にlfo処??
	...
	if(qq->lfo_wave == 0 || qq->lfo_wave == 4 || qq->lfo_wave == 5) {
		...
		if(al != 255) {
			if(--al == 0) {
				al = qq->time2;
				if(qq->lfo_wave != 4) {
					al += al;	// lfowave=0,5の???? timeを反転??２倍にす??
				}
				qq->time = al;
				qq->step = -qq->step;
				return;
...
int PMDWIN::soft_env_sub(QQ *qq)
{
	...
	if(qq->envf != 2) {
		// Decay
		if(qq->eenv_sr == 0) return 0;	// ＤＲ＝０の??は減衰しない
...
void PMDWIN::esm_sub(QQ *qq, int ah)
{
	if(--ah == 0) {
		...
		if(qq->eenv_arc > 0) {
			...
			qq->eenv_volume = 15;
			qq->eenv_count++;
			if(qq->eenv_sl != 15) return;		// SL=0の??合はすぐSRに
			qq->eenv_count++;
			return;
		} else {
	...
	if(--ah == 0) {
		//	[[[ Decay Rate ]]]
		if(qq->eenv_drc > 0) {	// 0以下の??合はカウントCHECK
	...
	if(--ah == 0) {
		//	[[[ Sustain Rate ]]]
		if(qq->eenv_src > 0) {	// 0以下の??合はカウントCHECK
	...
	//	[[[ Release Rate ]]]
	if(qq->eenv_rrc > 0) {	// 0以下の??合はカウントCHECK
...
void PMDWIN::silence(void)
{
	...
	opna.SetReg(0x110, 0x18);			// TIMERB/A/EOSのみbit変化あ??
...
{
	if(open_work.TimerAflag || open_work.TimerBflag) {
		pmdwork.music_flag |= 1;			//	TA/TB処??中は 実行しない
		return;
	}
...
{
	// TimerB = 0 に設定し、Timer Reset(曲の長さを毎回そ?暙┐?た??)
	open_work.tempo_d = 0;
	settempo_b();	
	opna.SetReg(0x27, 0);	// TIMER RESET(timerA,Bと??)
	...
	//	バッファ初??化
	//------------------------------------------------------------------------
	pos2 = (char *)wavbuf2;	// buf に余ってい??サンプ??の先頭位置
	us2	= 0;				// buf に余ってい??サンプ????
	upos = 0;				// 演奏開始からの??間(μsec)
...
void PMDWIN::play_init(void)
{
	...
	for(i = 0; i < 6; i++) { // word境界にバグ
		...
		FMPart[i].leng = 1;
		FMPart[i].keyoff_flag = -1;		// 現在keyoff??
	...
	for(i = 0; i < 3; i++) {
		...
		SSGPart[i].leng = 1;
		SSGPart[i].keyoff_flag = -1;	// 現在keyoff??
	...
	ADPCMPart.leng = 1;
	ADPCMPart.keyoff_flag = -1;		// 現在keyoff??
	...
	RhythmPart.leng = 1;
	RhythmPart.keyoff_flag = -1;	// 現在keyoff??
	RhythmPart.mdc = -1;			// MDepth Counter (無限)
	RhythmPart.mdc2 = -1;			// 同??
	RhythmPart._mdc = -1;			// 同??
	RhythmPart._mdc2 = -1;			// 同??
...
void PMDWIN::setint(void)
{
	...
	opna.SetReg(0x25, 0x00);			// TIMER A SET (9216μs固??)
	opna.SetReg(0x24, 0x00);			// ??番遅くて丁度いい
...
void PMDWIN::pcmstore(ushort pcmstart, ushort pcmstop, uchar *buf)
{
	...
	opna.SetReg(0x100, 0x01);
//	opna.SetReg(0x110, 0x17);	// brdy以外はマスク(=timer割????みは掛からない)
	opna.SetReg(0x110, 0x80);
...
//	PCM を検索ディ??クトリから検??
//		input
//			filename	: 検索す??ファイ??(ファイ??名部分のみ)
//			currentdir	: 最初に検索す????所があ??なら指??
//						  open_work.pcmdir よ??優先す??
//						  NULL なら無?? 
//		output			: 検索??果（フ??パス)
//						  見つからなかった?? NULL
//=============================================================================
	...
	if(*ppzfilename1 != '\0') {
		if((p = strchr(ppzfilename1, ',')) == NULL) {	// １つのみ
			if((p = strchr(ppzfilename1, '.')) == NULL) {
				strcat(ppzfilename1, ".PZI");			// とりあえず pzi にす??
			}
			...
			*p = '\0';
			...
			strcat(ppzfilename2, ".PZI");				// とりあえず pzi にす??
...
int PMDWIN::ppc_load2(char *filename)
{
	...
	size = (int)GetFileSize_s(filename);		// ファイ??サイズ
...
int PMDWIN::ppc_load3(uchar *pcmdata, int size)
{
	...
	if(memcmp(&tempbuf[30], &pcmends, sizeof(pcmends)) == 0) {
		return WARNING_PPC_ALREADY_LOAD;		// ??致した
	}
...
bool WINAPI PMDWIN::init(char *path)
{
	...
	open_work.rshot_bd = 0;					// ??ズム音源 shot inc flag (BD)
	open_work.rshot_sd = 0;					// ??ズム音源 shot inc flag (SD)
	open_work.rshot_sym = 0;				// ??ズム音源 shot inc flag (CYM)
	open_work.rshot_hh = 0;					// ??ズム音源 shot inc flag (HH)
	open_work.rshot_tom = 0;				// ??ズム音源 shot inc flag (TOM)
	open_work.rshot_rim = 0;				// ??ズム音源 shot inc flag (RIM)
	open_work.rdump_bd = 0;					// ??ズム音源 dump inc flag (BD)
	open_work.rdump_sd = 0;					// ??ズム音源 dump inc flag (SD)
	open_work.rdump_sym = 0;				// ??ズム音源 dump inc flag (CYM)
	open_work.rdump_hh = 0;					// ??ズム音源 dump inc flag (HH)
	open_work.rdump_tom = 0;				// ??ズム音源 dump inc flag (TOM)
	open_work.rdump_rim = 0;				// ??ズム音源 dump inc flag (RIM)
	...
	open_work.pcm86_vol = 0;		// PCM音量合??せ
	open_work._pcm86_vol = 0;		// PCM音量合??せ
	open_work.fade_stop_flag = 1;	// FADEOUT後MSTOPす??か FLAG
	...
	//----------------------------------------------------------------
	//	効果音/FMINT/EFCINTを初??化
	//----------------------------------------------------------------
	effwork.effon = 0;
	effwork.psgefcnum = 0xff;
	
	//----------------------------------------------------------------
	//	088/188/288/388 (同INT番号のみ) を初??設??
	//----------------------------------------------------------------
...
bool WINAPI PMDWIN::setpcmdir(char **path)
{
	...
	while(i < MAX_PCMDIR && path[i] != nullptr) {
		if(*path[i] == '\0') break;
...
//=============================================================================
//	PMD86 の PCM が PMDB2 互換かどうかを取得す??
//=============================================================================
...
void WINAPI PMDWIN::getpcmdata(short *buf, int nsamples)
{
	...
	do {
		if(nsamples - copysamples <= us2) {
			...
			opna.SetReg(0x27, open_work.ch3mode | 0x30);	// TIMER RESET(timerA,Bと??)
			...
			if(open_work.rate == open_work.ppzrate) {
				...
				carry = 0;
				for(i = 0; i < us2; i++) {		// ??波数変換(1 << 13 = 8192)
...
//=============================================================================
//	メモの取得（２バイト半角→半角文字に変換＋ESCシーケンスの??魚祝
//=============================================================================
...
//=============================================================================
//	メモの取得（ファイ??名から／半角文字に変換＋ESCシーケンスの??魚祝
//=============================================================================
...
int WINAPI PMDWIN::maskoff(int ch)
{
	...
	if(part_table[ch][0] < 0) {
		...
		if(open_work.MusPart[ch]->partmask == 0) return ERR_NOT_MASKED;	// マスクさ??ていない
											// 効果音でまだマスクさ??てい??
...
void WINAPI PMDWIN::setpos(int pos)
{
	...
	_pos = (__int64)pos * 1000;			// (ms -> usec への変換)
...
bool WINAPI PMDWIN::getlength(char *filename, int *length, int *loop)
{
	...
	*length = 0;
	...
	do {
		...
		opna.Count(us);
		upos += us;
		if(open_work.status2 == 1 && *length == 0) {	// ??ープ??
			*length = (int)(upos / 1000);
		} else if(open_work.status2 == -1) {			// ??ープなし終了??
			*length = (int)(upos / 1000);
			*loop = 0;
			mstop();
			...
			return true;
		} else if(getpos2() >= 65536) {		// 65536ク?暠奪?以上なら強制終了
			*length = (int)(upos / 1000);
			*loop = *length;
			return true;
		}
...
/*
	pmdwin2->setfmwait(0);			// 曲長計算高速化のた??
	pmdwin2->setssgwait(0);			// 曲長計算高速化のた??
	pmdwin2->setrhythmwait(0);		// 曲長計算高速化のた??
	pmdwin2->setadpcmwait(0);		// 曲長計算高速化のた??
*/
	return result;
}
